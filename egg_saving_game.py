...
class ScoreBoard():
    ...
    def initGUI(self):
        ...
        Label(self.parent, text="Lives:", font=("Times", 16, "bold")).grid(row=1, column=2, padx=35, pady=100, sticky=N+W)        
        Label(self.parent, textvariable=self.livesVar, font=("Times", 16, "bold")).grid(row=1, column=2, padx=60, pady=150, sticky=N+W)        
        
        # Score
        self.scoreVar = IntVar()
        Label(self.parent, text="Score:", font=("Times", 16, "bold")).grid(row=1, column=2, padx=35, pady=250, sticky=N+W)
        Label(self.parent, textvariable=self.scoreVar, font=("Times", 16, "bold")).grid(row=1, column=2, padx=50, pady=300, sticky=N+W)        
        
        # High score
        self.highScoreVar = IntVar()
        Label(self.parent, text="Highest Score:", font=("Times", 16, "bold")).grid(row=1, column=2, padx=0, pady=400, sticky=N+W)
        Label(self.parent, textvariable=self.highScoreVar, font=("Times", 16, "bold")).grid(row=1, column=2, padx=50, pady=450, sticky=N+W)
    ...
    def gameOver(self):
        self.saveScore()      
        if tkinter.messagebox.askyesno("Game Over", "Want to Play Aga1n ?"):
            self.reset()
        else:
             self.quit()
...
class ItemsFallingFromSky():
    ...
    def __init__(self,parent,canvas,player,board):
        ...
        self.goodItems = ["goldegg.gif","egg.png","egg.gif","eggcrack.gif"]
        self.badItems = ["snake1.gif","snake2.gif","owl.gif"]
...
class TheGame(ItemsFallingFromSky,ScoreBoard):
    ...
    def __init__(self,parent):
        ...
        self.parent.title("Welcome to Egg Saving Game")
    ...
    def keyMoving(self, event):        
        if (event.char == "1") and (self.canvas.coords(self.playerChar)[0] > 50):
            self.canvas.move(self.playerChar, -50, 0)            
        if (event.char == "3") and (self.canvas.coords(self.playerChar)[0] < 750):
            self.canvas.move(self.playerChar, 50, 0)
